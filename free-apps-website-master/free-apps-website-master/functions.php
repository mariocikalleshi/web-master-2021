<?php
	...
	// connect to database
	$db = mysqli_connect('localhost', 'mcikalleshi19', 'mc986shi', 'web22_mcikalleshi19');
	...
	function showUsers(){
		$conn = new mysqli('localhost', 'mcikalleshi19', 'mc986shi', 'web22_mcikalleshi19');
		// Check connection
	...
	function showApps(){
		...
		$sql = "SELECT a_id, a_name, photo, description, link, category FROM apps ORDER BY a_id DESC";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			// output data of each row
